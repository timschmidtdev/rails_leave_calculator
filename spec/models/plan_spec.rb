...
RSpec.describe Plan, type: :model do
  ...
  let(:plan) { employee.plans.create!(plan_type: plan_type, start: date, length: length, unit: unit)}
  ...
  describe "attributes" do
    it "has type, start, length, and unit attributes" do
      expect(plan).to have_attributes(plan_type: plan_type, start: date, length: length, unit: unit)
    end
  end
end
